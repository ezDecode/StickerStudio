...
import { generateSticker, processImage, detectSubject, editSticker, enhancePromptForComedy, convertToWhatsAppFormat, STICKER_STYLES, StickerStyleKey } from '../services/geminiService';
import { saveAutosave, getAutosave, clearAutosave } from '../services/storage';
import Button from '../components/Button';
import ImageUpload from '../components/ImageUpload';
import { Sticker } from '../types';
import { Download, Sparkles, ScanEye, PenTool, Type, Share2, Trash2, Wand2, X, Check, ThumbsUp, ThumbsDown, AlertTriangle, Globe, ExternalLink, Search, Zap, MessageCircle, Loader2, Palette } from 'lucide-react';
...
const StickerMaker: React.FC<StickerMakerProps> = ({ onStickerCreated, initialSticker }) => {
  ...
  // Track if user has downloaded/shared the current creation
  ...
  // 1. Load initial sticker (Edit Mode) OR Autosave (Session Restore)
  useEffect(() => {
    const loadSession = async () => {
      if (initialSticker) {
        ...
        if (saved && saved.generatedSticker) {
          ...
          // If we restored from autosave, we assume it's unsaved work unless proven otherwise
  ...
  // 2. Autosave whenever state changes (Debounced slightly by React render cycle)
  useEffect(() => {
  ...
  // 3. Prevent accidental exit if sticker exists and hasn't been downloaded
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // --- AUTOMATIC COMEDY ENHANCEMENT STEP ---
      // We force a search if there is any prompt to find context/memes
      let finalPrompt = prompt;
      if (prompt.trim()) {
        try {
          setLoadingMsg("Applying 'Funny' filters...");
          const enhanced = await enhancePromptForComedy(prompt);
          if (enhanced.text !== prompt) {
             console.log("Enhanced Prompt:", enhanced.text);
             finalPrompt = enhanced.text;
          }
          if (enhanced.sources.length > 0) {
             setGroundingSources(enhanced.sources);
          }
        } catch (e) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      // Save edited version as new sticker
      const newSticker: Sticker = {
        id: Date.now().toString(),
        url: result,
      ...
      setError("Failed to edit sticker. " + (err.message || "Please try again."));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Specifically for sharing to WhatsApp as a real "Sticker" (WebP format)
  const handleShare = async () => {
    if (!generatedSticker) return;
    
    setIsSharing(true);
    try {
      // 1. Convert to WhatsApp specific format (512x512, WebP, <100KB)
      const blob = await convertToWhatsAppFormat(generatedSticker);
      
      // 2. Create a File object with .webp extension
  ...
  const handleQuickFeedback = (type: 'up' | 'down') => {
    ...
    console.log(`User voted ${type} on sticker`);
  };
  ...
  const resetAll = async () => {
    ...
    setHasDownloaded(false);
    setGroundingSources([]);
    await clearAutosave(); // Clear persistence
  };
          ...
          <div className="bg-[#0c0c0e] border border-white/10 rounded-3xl p-6 shadow-xl flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
               ...
               {(selectedImage || generatedSticker) && (
                 <button onClick={resetAll} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1 transition-colors">
            ...
            <div className="space-y-5">
                {/* Style Indicator - Simplified for Single Style */}
                <div className="space-y-2">
                ...
                {/* Caption Field - Always Visible & Enhanced */}
                <div className="space-y-2 animate-in fade-in">
                  ...
                  <div className="relative group">
                      {/* Gradient Highlight for text field */}
                 ...
                 <div className="flex items-center gap-2">
                    {generatedSticker && !isLoading && (
                      <>
                        ...
                        {/* SHARE BUTTON - The Fix for WhatsApp */}
                        <button 
                           onClick={handleShare}
                           disabled={isSharing}
              ...
              <div className="relative z-10 w-full h-full flex flex-col items-center justify-center p-10">
                {generatedSticker ? (
                  <div className="relative group w-full h-full flex flex-col items-center justify-center animate-in zoom-in-95 duration-500">
                     ...
                     {/* Floating Action Bar (Only show if NOT editing and NOT loading) */}
                     {!isEditing && !isLoading && (
